...
export const google = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            const randomUserName = req.body.username.split(' ').join('').toLowercase() + Math.random().toString(36).slice(-4)
            const newUser = new User({username: randomUserName, mail: req.body.email, password: hashedPassword, avatar: req.body.photo})
            await newUser.save()
